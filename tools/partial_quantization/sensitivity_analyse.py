...
def quant_sensitivity_analyse(model_ptq, evaler):
    ...
    for k, m in model_ptq.named_modules():
        ...
        print(eval_result)
        print(
            "Quantize Layer {}, result mAP0.5 = {:0.4f}, mAP0.5:0.95 = {:0.4f}".format(
                k, eval_result[0], eval_result[1]
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf", type=str, default="../../configs/repopt/yolov6s_opt.py", help="model config"
    )
    ...
    parser.add_argument(
        "--calib-weights", type=str, default=None, help="weights with calibration parameter"
    )
    ...
    print(
        "Full Precision model mAP0.5={:.4f}, mAP0.5_0.95={:0.4f}".format(orig_mAP[0], orig_mAP[1])
    )
    ...
    print(
        "Post Training Quantization model mAP0.5={:.4f}, mAP0.5_0.95={:0.4f}".format(
            quant_mAP[0], quant_mAP[1]
        )
    )
    ...
    qfile = "{}_quant_sensitivity_{}_calib.txt".format(
        os.path.basename(args.weights).split(".")[0], args.batch_size * args.batch_number
    )
