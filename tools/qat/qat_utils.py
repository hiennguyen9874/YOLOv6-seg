...
def ptq_calibrate(model, train_loader, cfg):
    ...
    with torch.no_grad():
        ...
        compute_amax(
            model,
            method=cfg.ptq.histogram_amax_method,
...
def qat_init_model_manu(model, cfg, args):
    ...
    conv2d_input_default_desc = QuantDescriptor(
        num_bits=cfg.ptq.num_bits, calib_method=cfg.ptq.calib_method
    )

    convtrans2d_weight_default_desc = (
        tensor_quant.QUANT_DESC_8BIT_CONVTRANSPOSE2D_WEIGHT_PER_CHANNEL
    )
    convtrans2d_input_default_desc = QuantDescriptor(
        num_bits=cfg.ptq.num_bits, calib_method=cfg.ptq.calib_method
    )
