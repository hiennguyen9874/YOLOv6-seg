        ...
        if node.op_type == "Add":
            ...
            for input_id in node.input:
                ...
                if prev_node is not None:
                    if (
                        prev_node.op_type in ["BatchNormalization", "Conv"]
    ...
    for add_node in pattern:
        ...
        for node in prev_add_node_list:
            if node.op_type == "BatchNormalization":
                ...
                assert (
                    len(prev_node_list) == 1 and prev_node_list[0].op_type == "Conv"
        ...
        for node in conv_node_list:
            ...
            assert (
                dequant_node is not None and quant_node is not None
            ), "Conv horizon fusion pattern not match"
...
def onnx_add_insert_qdqnode(onnx_model):
    ...
    for node_id, node in enumerate(graph.node):
        if node.op_type == "Add":
            ...
            if (
                len(same_input_node_list) == 1
                and same_input_node_list[0].op_type == "QuantizeLinear"
            ):
                prev_add_node = search_node_by_output_id(nodes, same_input)
                dequant_node = get_next_node(nodes, same_input_node_list[0])[0]
                patterns.append(
                    (node, prev_add_node, same_input_node_list[0], dequant_node, same_input)
                )
    print(patterns)
