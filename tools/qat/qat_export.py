...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--quant-weights",
        type=str,
        default="./yolov6s_v2_reopt_qat_43.0.pt",
        help="calib weights path",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--eval-yaml",
        type=str,
        default="../partial_quantization/eval.yaml",
    ...
    model = load_checkpoint(
        args.weights, map_location=device, inplace=args.inplace, fuse=args.fuse_bn
    )
    ...
    if args.export_batch_size is None:
        ...
        export_file = args.quant_weights.replace(
            ".pt", "_bs{}.onnx".format(args.export_batch_size)
        )  # filename
        if args.graph_opt:
            export_file = export_file.replace(".onnx", "_graph_opt.onnx")
        if args.end2end:
