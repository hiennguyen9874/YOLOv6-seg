...
def build_engine_from_onnx(
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
        ...
        try:
            ...
            raise AssertionError("Parsing failed on line {} in statement {}".format(line, text))
...
def main():
    ...
    parser.add_argument(
        "-d", "--dtype", type=str, required=True, help="one type of int8, fp16, fp32"
    )
    parser.add_argument(
        "--qat",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=[640, 640],
        help="image size of model input, the order is: height width",
    )
    parser.add_argument(
        "--batch-size", type=int, default=128, help="batch size for training: default 64"
    )
    parser.add_argument(
        "--num-calib-batch", default=6, type=int, help="Number of batches for calibration"
    )
    parser.add_argument(
        "--calib-img-dir",
        default="../coco/images/train2017",
        type=str,
        help="Number of batches for calibration",
    )
    parser.add_argument(
        "--calib-cache",
        default="./yolov6s_calibration.cache",
        type=str,
    ...
    if args.dtype == "int8" and not args.qat:
        engine_path = args.model.replace(
            ".onnx", "-int8-{}-{}-minmax.trt".format(args.batch_size, args.num_calib_batch)
        )
