...
def parse_args():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=1, help="batch size for training: default 64"
    )
    parser.add_argument(
        "-c", "--category-num", type=int, default=80, help="number of object categories [80]"
    )
    parser.add_argument("--img-size", nargs="+", type=int, default=[640, 640], help="image size")
    parser.add_argument(
        "-m", "--model", type=str, default="./weights/yolov5s-simple.trt", help=("trt model path")
    )
    parser.add_argument(
        "--conf-thres", type=float, default=0.03, help="object confidence threshold"
    )
...
def generate_results(
    ...
    for _ in pbar:
        imgs = torch.randn(
            (batch_size, 3, 640, 640), dtype=torch.float32, device=torch.device("cuda:0")
        )
        source_imgs = []
        image_names = []
        shapes = []
