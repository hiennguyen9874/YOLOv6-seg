...
def parse_args():
    ...
    parser.add_argument(
        "--imgs_dir",
        type=str,
        default="../coco/images/val2017",
        help="directory of validation dataset images.",
    )
    parser.add_argument(
        "--labels_dir",
        type=str,
        default="../coco/labels/val2017",
    ...
    parser.add_argument("--batch_size", type=int, default=1, help="batch size of evaluation.")
    parser.add_argument("--img_size", nargs="+", type=int, default=[640, 640], help="image size")
    parser.add_argument(
        "--model", "-m", type=str, default="./weights/yolov5s.trt", help=("trt model path")
    )
    parser.add_argument("--conf_thres", type=float, default=0.03, help="confidence threshold")
    parser.add_argument("--iou_thres", type=float, default=0.65, help="IOU threshold for NMS")
    parser.add_argument(
        "--class_num",
        type=int,
        default=3,
        help="class list for general datasets that must be specified",
    )
    parser.add_argument(
        "--is_coco",
        action="store_true",
        help="whether the validation dataset is coco, default is False.",
    )
    parser.add_argument(
        "--shrink_size",
        type=int,
        default=4,
        help="load img with size (img_size - shrink_size), for better performace.",
    )
    parser.add_argument(
        "--visualize", "-v", action="store_true", default=False, help="visualize demo"
    )
    parser.add_argument(
        "--num_imgs_to_visualize", type=int, default=10, help="number of images to visualize"
    )
    parser.add_argument(
        "--do_pr_metric", action="store_true", help="use pr_metric to evaluate models"
    )
    ...
    parser.add_argument(
        "--is_end2end", action="store_true", help="whether the model is end2end (build with NMS)"
    )
...
def generate_results(
    ...
    results = []
    pbar = tqdm(
        range(math.ceil(len(valid_images) / batch_size)), desc="TRT-Model test in val datasets."
    )
    idx = 0
    num_visualized = 0
    stats = []
    ...
    for _ in pbar:
        ...
        for i in range(batch_size):
            ...
            with open(label_path, "r") as f:
                target = [x.split() for x in f.read().strip().splitlines() if len(x)]
        ...
        for j in range(len(shapes)):
            pred = processor.post_process(
                output[j].unsqueeze(0), shapes[j], conf_thres=conf_thres, iou_thres=iou_thres
            )
            ...
            if do_pr_metric:
                ...
                if nl:
                    ...
                    labelsn = torch.cat(
                        (torch.from_numpy(labels[:, 0:1]).cpu(), torch.from_numpy(tbox).cpu()), 1
                    )  # native-space labels
    ...
    if args.do_pr_metric:
        ...
        if len(stats) and stats[0].any():
            ...
            p, r, ap, f1, ap_class = ap_per_class(
                *stats, plot=args.plot_curve, save_dir=args.save_dir, names=model_names
            )
            ...
            nt = np.bincount(
                stats[3].astype(np.int64), minlength=len(model_names)
