...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--data", type=str, default="./data/coco.yaml", help="dataset yaml file path."
    )
    parser.add_argument(
        "--weights", type=str, default="./yolov6s.engine", help="tensorrt engine file path."
    )
...
@torch.no_grad()
def run(
    data, weights=None, batch_size=32, img_size=640, task="val", device="", save_dir="", name=""
):
    """
    ...
    # init
    val = Evaler(data, batch_size, img_size, None, None, device, False, save_dir)
