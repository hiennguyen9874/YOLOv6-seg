...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--img-size", nargs="+", type=int, default=[320, 320], help="Image size of height and width"
    )
    parser.add_argument("--max-stride", type=int, default=64, help="Max stride of yolov6 model")
...
def yolov6_decode(
    feats: List[ndarray], conf_thres: float, iou_thres: float, num_labels: int = 80, **kwargs
):
    ...
    if MINOR >= 7:
        indices = cv2.dnn.NMSBoxesBatched(
            proposal_boxes, proposal_scores, proposal_labels, conf_thres, iou_thres
        )
    elif MINOR == 6:
...
def main(args: argparse.Namespace):
    ...
    mat_in = ncnn.Mat.from_pixels_resize(img, ncnn.Mat.PixelType.PIXEL_BGR2RGB, img_w, img_h, w, h)
