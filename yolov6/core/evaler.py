...
class Evaler:
    ...
    def predict_model(
        self, model, dataloader, task, issolo=False, weight_nums=66, bias_nums=1, dyconv_channels=66
    ):
        """Model prediction
        ...
        for i, (imgs, targets, paths, shapes, masks) in enumerate(pbar):
            ...
            if not issolo:
                loutputs = non_max_suppression_seg(
                    toutputs, self.conf_thres, self.iou_thres, multi_label=True
                )
            else:
                loutputs = non_max_suppression_seg_solo(
                    toutputs, self.conf_thres, self.iou_thres, multi_label=True
                )
            protos = toutputs[1][0]
            segments = []
            ...
            if not issolo:
                segments = [
                    ...
                    for li in range(len(loutputs))
                ]
            else:
                segments = [
                    self.handle_proto_solo(
                        [protos[li].reshape(1, *(protos[li].shape[-3:]))],
                        segconf[li],
                        imgs.shape[-2:],
                        weight_sums=weight_nums,
                        bias_sums=bias_nums,
                        dyconv=dyconv_channels,
                    )
                    for li in range(len(loutputs))
                ]
            ...
            for si, (pred, pred_masks) in enumerate(zip(eval_outputs, segments)):
                ...
                if nl:
                    ...
                    correct_masks = process_batch(
                        predn, labelsn, iouv, pred_masks, gt_masks, overlap=False, masks=True
                    )
                    if self.plot_confusion_matrix:
                ...
                stats.append(
                    (correct_masks.cpu(), correct.cpu(), pred[:, 4].cpu(), pred[:, 5].cpu(), tcls)
                )
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                nt = np.bincount(
                    stats[4].astype(np.int64), minlength=model.nc
    ...
    def parse_dynamic_params(self, flatten_kernels, weight_nums, bias_nums, dyconv_channels):
        ...
        params_splits = list(
            torch.split_with_sizes(flatten_kernels, weight_nums + bias_nums, dim=1)
        )
        weight_splits = params_splits[:n_layers]
        bias_splits = params_splits[n_layers:]
        for i in range(n_layers):
            if i < n_layers - 1:
                weight_splits[i] = weight_splits[i].reshape(n_inst * dyconv_channels, -1, 1, 1)
                bias_splits[i] = bias_splits[i].reshape(n_inst * dyconv_channels)
            else:
    ...
    def handle_proto_solo(
        ...
        weights, biases = self.parse_dynamic_params(
            confs, weight_nums=weight_sums, bias_nums=bias_sums, dyconv_channels=dyconv
        )
        n_layers = len(weights)
        for i, (weight, bias) in enumerate(zip(weights, biases)):
            x = F.conv2d(proto, weight, bias=bias, stride=1, padding=0, groups=1)
            if i < n_layers - 1:
                x = F.relu(x)
    ...
    def handle_proto_test(proto_list, oconfs, imgshape, img_orishape=None):
        ...
        s = proto.shape[-2:]
        seg = (confs @ proto.reshape(proto.shape[0], proto.shape[1], -1)).reshape(
