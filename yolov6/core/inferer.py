...
class Inferer:
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img:
                        ...
                        img_ori = self.plot_box_and_label(
                            img_ori,
    ...
    def rescale_mask(ori_shape, masks, target_shape):
        ...
        masks = masks[
            :, padding[1] : ori_shape[0] - padding[1], padding[0] : ori_shape[1] - padding[0]
        ]
    ...
    def handle_proto(proto_list, oconfs, imgshape, det):
        ...
        for i, proto in enumerate([protos, protos, protos]):
            ...
            if tconfs.shape[0] == 0:
                continue
            tseg = (tconfs @ proto.reshape(proto.shape[0], proto.shape[1], -1)).reshape(
                proto.shape[0], tconfs.shape[1], *s
            )
            print("a:")
    ...
    def plot_box_and_label(
        ...
        if segment is not None:
            ...
            colr = (
                colr.reshape(1, 3)
                .repeat((image.shape[0] * image.shape[1]), axis=0)
                .reshape(image.shape[0], image.shape[1], 3)
            )
            image = cv2.addWeighted(
                image,
                1,
                (colr * segment.reshape(*segment.shape[:2], 1)).astype(image.dtype),
                0.8,
                1,
            )
        return image
