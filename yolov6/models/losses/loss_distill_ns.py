...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        if num_pos > 0:
            ...
            pred_bboxes_lrtb_pos = torch.masked_select(pred_bboxes_lrtb, bbox_mask).reshape([-1, 4])
            ...
            loss_iou_lrtb = self.iou_loss(pred_bboxes_lrtb_pos, target_bboxes_pos) * bbox_weight
            ...
            if self.use_dfl:
                ...
                d_loss_dfl = (
                    self.distill_loss_dfl(pred_dist_pos, t_pred_dist_pos, temperature) * bbox_weight
                )
                if target_scores_sum == 0:
